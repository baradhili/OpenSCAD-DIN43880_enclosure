...
*/
TOPTHICKNESS = 2.2;        ///< heavy TOP thickness
...
//box display options - bitwise functions
PRINTBOX = 1; //print box only
PRINTLID = 2; //print lid only


//! @privatesection
include <omdl/omdl-base.scad>; //TODO - get rid of the need for this and use the arrays lib instead
...
@fn  simpleDINBox(size_hm, top = DINTOP, width= DINWIDTHM, leftless=0, rigthless=0, uclip = xauto, lidHole = -7, lidThick = TOPTHICKNESS, lidFill = 100, towerHole = -58, bottomThick = TOPTHICKNESS, bottomFill = 100, boxThick = NOTOPTHICKNESS, lidStyle = LSIN, x=0, y=0, rot = norot)
  ...
  @param lidFill  the fill factor for the lid (default 100).
*/
module simpleDINBox(size_hm, top = DINTOP, width= DINWIDTHM, leftless=0, rigthless=0, uclip = xauto, lidHole = -7, lidThick = TOPTHICKNESS, lidFill = 100, towerHole = -58, bottomThick = TOPTHICKNESS, bottomFill = 100, boxThick = NOTOPTHICKNESS, lidStyle = LSIN, x=0, y=0, rot = norot, print=255, OPTION_BOXPRINT=true){
    //box display options - bitwise functions
    PRINTBOX = 1; //print box only
    PRINTLID = 2; //print lid only
    //vars
   ...
   _towList = get_DINvertex2tower(_useArray, towerHole, _dt);      // get tower positions
    ...
    //box
    if(bitwise_and(print,PRINTBOX,bv=1)){
        translate([x,y,0])rotate(rot)union(){
    ...
    //lid
    if(bitwise_and(print,PRINTLID,bv=1)){
