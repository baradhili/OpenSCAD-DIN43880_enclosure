...
include <../lib/e3DHW_base_lib.1.3.scad>
include <../lib/e3DHW_array_lib.1.3.scad>
include <../lib/e3DHW_hardware_data.1.3.scad>
include <../lib/e3DHW_addon_base.1.3.scad>
include <../lib/e3DHW_DIN_rail_lib.1.3.scad>
include <../lib/e3DHW_DIN_boxes_lib.1.3.scad>
include <../lib/e3DHW_addon_box.1.3.scad>
...
module din_DSP4(){
 include <../lib/e3DHW_addon_terminal.1.3.scad> //required for this project
...
use_clip = xauto;
clip_length=_hm*2*18; //not sure how this works.. maybe only has an effect on certain clip types anyway its the length of the box along the rail
clip_width=80; //this is width across the rail of the rail
   //
    length = get_H(_hm);
    difference(){
        union(){
           simpleDINBox(_hm, width= DINWIDTHS, top = DINHTOP, boxThick = TOPTHICKNESS, lidStyle = LSIN, bottomFill=DEFAULTFILL, uclip = xauto, leftless=-8);
           // here addons
           // box for Sonoff
     ...
     // aeration
      ...
      //make clip
      //clip = get_Clip(use_clip, clip_length,["SPF","MSX"]);  // defauts: SPF (4<= length < 26) and MSX( length >= 26)
    //translate([get_ClipTrnsX(clip, clip_width), 0, 0]) do_DINClip(clip, length, get_ClipWidth(clip, clip_width));
